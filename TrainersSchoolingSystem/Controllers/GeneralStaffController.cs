...
namespace TrainersSchoolingSystem.Controllers
{
    ...
    public class GeneralStaffController : Controller
    {
        ...
        public ActionResult GetCurrentGeneralStaff()
        {
            try
            {
                var teachers = db.Database.SqlQuery<TeacherSP>("GetCurrentGeneralStaff").ToList();
        ...
        private string GetPaySlip(int StaffIdId)
        {
            string data = "";

            try
            {
                var lastdate = db.PaidFees.Where(x => x.StudentId == StaffIdId
                ...
                if (lastdate.HasValue && (DateTime.Now - lastdate).Value.TotalDays < 28 &&
                    ...
                    return "";
                SqlParameter staffId = new SqlParameter("@StaffId", StaffIdId);
                var PaySlipData = db.Database.SqlQuery<StaffPaySlipModel>("exec GeneratePaySlips @StaffId", staffId).FirstOrDefault();
                ...
                var lastPayment = db.SalaryPayments.Where(x => x.StaffId == StaffIdId).OrderByDescending(x => x.CreatedDate).FirstOrDefault();
                var userid = db.TrainerUsers.Where(x => x.Username == User.Identity.Name).FirstOrDefault().TrainerUserId;
